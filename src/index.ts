...
if (!commands.includes(command as (typeof commands)[number])) { /* ... handle invalid command ... */ }
const neonApiKey = command === 'start' ? NEON_API_KEY_FROM_ENV : neonApiKeyFromArgs;
if (command === 'start' && !neonApiKey) { /* ... warn missing API key ... */ }
if (command === 'init') { /* ... handle init ... */ }
...
const httpServer = http.createServer(async (req, res) => {
    ...
    // 1. SSE Connection Endpoint (e.g., GET /mcp-sse)
    ...
    // 2. Incoming Message Endpoint (e.g., POST /mcp-post?sessionId=...)
        ...
        if (!sessionId) { /* ... handle missing sessionId ... */ return; }
        const transport = activeConnections.get(sessionId);
        if (!transport) { /* ... handle unknown session ... */ return; }
        try {
            ...
            if (!res.writableEnded) { /* ... handle error ... */ }
        }
    } else {
...
async function main() {
  httpServer.listen(PORT, HOST, () => {
    ...
    console.log(`Client POSTs expected at ${MCP_POST_PATH}?sessionId=...`); // Clarified POST path
